...
volatile uint32_t bolgenos::jiffies = 0;


static void __sleep(uint32_t ticks_timeout)
{
	uint32_t end_of_sleep = bolgenos::jiffies + ticks_timeout;
	while (bolgenos::jiffies < end_of_sleep)
	{
		x86::halt_cpu();
	}
}
...
void bolgenos::sleep_ms(uint32_t ms) {
...
uint32_t bolgenos::ms_to_ticks(uint32_t ms) {
	uint32_t ticks = 0;
